...
check_1password() {
    ...
    echo ""
    echo "---- 1Password check: ${phase} ----"

    if path=$(command -v 1password 2>/dev/null); then
        hits+=("${path}")
        found=true
    fi

    if path=$(command -v op 2>/dev/null); then
        hits+=("${path}")
        found=true
    fi
    ...
    for path in \
        ...
        /usr/bin/op \
        /opt/1Password \
        /usr/share/applications/*1[Pp]assword* \
        /usr/share/applications/*1Password* \
        /var/lib/flatpak/app/com.1password.OnePassword \
        "${SCRIPT_HOME}/.local/share/flatpak/app/com.1password.OnePassword"
    do
        if [[ -e "${path}" ]]; then
            hits+=("${path}")
            found=true
    ...
    if [[ "${found}" == "true" ]]; then
        printf '  [FOUND] %s\n' "${hits[@]}"
        die "1Password detected ${phase}. Stopping install so this phase can be investigated."
    fi

    info "No 1Password detected ${phase}."
}
...
main() {
    ...
    ask_cinnamon
    preflight
    check_1password "before install phases"
    configure_dnf
    if [[ "${INSTALL_CINNAMON}" == "true" ]]; then
        install_cinnamon
        check_1password "after cinnamon"
    else
        check_1password "after cinnamon skipped"
    fi
    ensure_display_manager
    check_1password "after lightdm"
    setup_repos
    check_1password "after repo setup"
    install_packages
    check_1password "after package install"
    ensure_niri_session_file
    configure_niri
    configure_portals
    configure_system_env
    configure_gtk_theme
    install_noctalia_polkit
    check_1password "after noctalia polkit plugin"
    offer_lgl_tools
    display_banner
}
